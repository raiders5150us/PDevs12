...
namespace ProjectDevs.Api.Controllers
{
    ...
    public class StoriesController : BaseApiController
    {
        ...
        [Route("master/status")]
        public IHttpActionResult GetProjectStatus() => Ok(_cacheService.StoryStatusNames.Select(s => new { Value = s.Key, Text = s.Value }));

        [Route("ddl")]
        public IHttpActionResult GetProjectStoriesDdl(int projectId)
        {
            var stories = _repoFactory.ProjectStories.GetProjectStories(projectId);
            var options = stories?.Select(s => new { Value = s.StoryId, Text = $"As a {s.F1}, I want to {s.F2}, So I can {s.F3}" });
            return Ok(options);
        }
